...
# define AMAX            19     //  getallen  x + y Ã rad  voor |x| <= AMAX 
# define BMAX            19     //                     en  voor |y| <= BMAX 
# define QNORM(x, y)    abs(x * x + coef2 * (y * y)) // norm of x + y Ã rad 
...
{	int i; char signtoken[] = "-o+"; 
	printf("\n\n Q(V%d)  chi ", rad); 
	for (i = 0; i < period; i++) printf("%c", signtoken[qchar[i] + 1]);
	printf("\n");
}   /*  end heading  */
	
void picture()  /*  prints picture of primes and units  */
{	int a, b, qnm; extern int period, coef2; coef2 = - discriminant / 4;
	for (b = - BMAX; b <= BMAX; b++) 
	{	printf("\n");
		for (a = - AMAX; a <= AMAX; a++)  
			// too big or prime or unit or composite number:
			if ((qnm = QNORM(a, b)) >= maxnorm) printf("  ");
			else if (inset(qnm, prinorm)) printf(" %i", qnm % 10);
