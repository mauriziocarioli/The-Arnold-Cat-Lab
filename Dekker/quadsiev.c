...
{	return 2 * size * WORD;}  /* end capacity, doubled by space saving */
	
void makedelta(int qchar[])
/*  deltac[period] = difference of odd numbers of positive character */
{	int c, cnext; extern int period; extern int deltac[];
	for (c = period -1, cnext = period +1; c > 0; c -= 2)
		if (qchar[c] == 1) { deltac[c] = cnext-c; cnext = c; }
}   /*  end makedelta  */
...
{	extern int pneven; pneven = i;}  /* end addeven, for space saving */
	
int inset(int i, short prinorm[])
/*  returns 1 (true) if i is a prime norm, or 0 (false) otherwise */
{	extern int pneven;
	if (i % 2 == 0) return i == pneven; else return INSET(i, prinorm); 
...
{	int i, k, m, im, maxim;
	for (i = 0; i < size; i++) prinorm[i] = 0;   /* Initialize prinorm */
	addeven(fac[0]);  /* add the even prime norm, 2 or 4, space saving */
	for (k = 1; k < nf; k++)  { i = fac[k]; ADD(i, prinorm); } 
	for (i = next(1); i < max; i = next(i)) ADD(i, prinorm);
